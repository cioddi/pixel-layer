...
exports.SOUTH_POLE_Y = exports.NORTH_POLE_Y = void 0;
exports.subdividePolygon = subdividePolygon;
exports.getFillGranularity = getFillGranularity;
exports.subdivideVertexLine = subdivideVertexLine;
exports.fixWindingOrder = fixWindingOrder;
exports.scanlineTriangulateVertexRing = scanlineTriangulateVertexRing;
const point_geometry_1 = __importDefault(require("@mapbox/point-geometry"));
...
exports.NORTH_POLE_Y = -32768;
exports.SOUTH_POLE_Y = 32767;
class Subdivider {
    ...
    _ensureNoPoleVertices() {
        ...
        for (let i = 0; i < flattened.length; i += 2) {
            ...
            if (vy === exports.NORTH_POLE_Y) {
                // Move slightly down
                flattened[i + 1] = exports.NORTH_POLE_Y + 1;
            }
            if (vy === exports.SOUTH_POLE_Y) {
                // Move slightly down
                flattened[i + 1] = exports.SOUTH_POLE_Y - 1;
            }
        }
    }
    /**
    ...
    _generatePoleQuad(indices, i0, i1, v0x, v1x, poleY) {
        const flip = (v0x > v1x) !== (poleY === exports.NORTH_POLE_Y);
        if (flip) {
            indices.push(i0);
            indices.push(i1);
        ...
        for (let primitiveIndex = 2; primitiveIndex < numIndices; primitiveIndex += 3) {
            ...
            if (north) {
                if (v0y === northEdge && v1y === northEdge) {
                    this._generatePoleQuad(indices, i0, i1, v0x, v1x, exports.NORTH_POLE_Y);
                }
                if (v1y === northEdge && v2y === northEdge) {
                    this._generatePoleQuad(indices, i1, i2, v1x, v2x, exports.NORTH_POLE_Y);
                }
                if (v2y === northEdge && v0y === northEdge) {
                    this._generatePoleQuad(indices, i2, i0, v2x, v0x, exports.NORTH_POLE_Y);
                }
            }
            if (south) {
                if (v0y === southEdge && v1y === southEdge) {
                    this._generatePoleQuad(indices, i0, i1, v0x, v1x, exports.SOUTH_POLE_Y);
                }
                if (v1y === southEdge && v2y === southEdge) {
                    this._generatePoleQuad(indices, i1, i2, v1x, v2x, exports.SOUTH_POLE_Y);
                }
                if (v2y === southEdge && v0y === southEdge) {
                    this._generatePoleQuad(indices, i2, i0, v2x, v0x, exports.SOUTH_POLE_Y);
                }
            }
        }
    }
    ...
    subdividePolygonInternal(polygon, generateOutlineLines) {
        ...
        try {
            ...
            const earcutResult = (0, earcut_1.default)(flattened, holeIndices);
...
function getFillGranularity(zoom) {
    const baseGranularity = 128;
    const minGranularity = 2;
    const divisor = Math.max(1, 1 << Math.max(0, zoom));
    return Math.max(Math.floor(baseGranularity / divisor), minGranularity, 1);
}
/**
...
function subdivideVertexLine(linePoints, granularity, isRing = false) {
    ...
    finalLineVertices.push(new point_geometry_1.default(linePoints[0].x, linePoints[0].y));
    ...
    for (let pointIndex = 0; pointIndex < lastIndex; pointIndex++) {
        ...
        while (true) {
            ...
            if ((realDistanceToBoundaryX < realDistanceToBoundaryY && dirXnonZero) || !dirYnonZero) {
                ...
                const next = new point_geometry_1.default(lastPointX, Math.round(lastPointY));
                ...
                const next = new point_geometry_1.default(Math.round(lastPointX), lastPointY);
                if (finalLineVertices[finalLineVertices.length - 1].x !== next.x ||
                    finalLineVertices[finalLineVertices.length - 1].y !== next.y) {
                    finalLineVertices.push(next);
                }
        ...
        const last = new point_geometry_1.default(lineVertex1x, lineVertex1y);
